                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            return [4 /*yield*/, dbo.collection("servers").findOne({ id: message.guild.id })];
                        case 1:
                            result = _a.sent();
                            if (result === null || result === undefined) {
                                dbo.collection("servers").insertOne({ id: message.guild.id, prefix: "-", loop: false }, function (err, res) {
                                    if (err)
                                        throw err;
                                    return "-";
                                });
                            ...
                            // dbo.collection("users").updateOne({ name: userId}, { $set: {date: date}}, function(err, res)
                                ...
                                dbo.collection("servers").updateOne({ id: message.guild.id }, { $set: { prefix: "-" } }, function (err, res) {
                                    if (err)
                                        throw err;
                                    return "-";
                                });
